...
export class TrackDetailsComponent implements OnInit, OnDestroy {
  ...
  public pieChartLegend = false;
	...
	public piechatLabels2: Label[] = ['Skintone 1 ✋🏻', 'Skintone 2 ✋🏼', 'skintone 3 ✋🏽', 'Skintone 4 ✋🏾', 'Skintone 5 ✋🏿'];
	public pieChartData2: SingleDataSet = [30, 50, 40, 10, 5];
  public pieChartType2: ChartType = 'pie';
	public pieChartPlugins2 = [];
	public pieChartColors2 = [
    {
      backgroundColor: ['rgb(127,126,120)', 'rgb(236,231,205)', 'rgb(134, 10,50)', 'rgb(192,128,104)', 'rgb(12,18,14)'],
    },
	];
  ...
  public barChartLabels: Label[] = ['ST1 ✋🏻', 'ST2 ✋🏼', 'ST3 ✋🏽', 'ST4 ✋🏾', 'ST5 ✋🏿'];
  ...
  search1() {
		...
		if (this.name3 === '') {
			...
			this.newFans = this.newFans.filter(resp => String(resp.name).toLocaleLowerCase().match(this.name3.toLocaleLowerCase()));
		}
	}
  ...
  search3() {
		...
		if (this.name3 === '') {
			...
			this.newSuperFans = this.newSuperFans.filter(resp => String(resp.name).toLocaleLowerCase().match(this.name3.toLocaleLowerCase()));
		}
  }
  ...
  search4() {
		...
		if (this.name4 === '') {
			...
			this.newSpeakers = this.newSpeakers.filter(resp => String(resp.name).toLocaleLowerCase().match(this.name4.toLocaleLowerCase()));
		}
  }
  ...
  search5() {
		...
		if (this.name5 === '') {
			...
			this.newModerators = this.newModerators.filter(resp => String(resp.name).toLocaleLowerCase().match(this.name5.toLocaleLowerCase()));
		}
	}
  ...
  search6() {
		...
		if (this.name6 === '') {
			...
			this.newSpdData = this.newSpdData.filter(resp => String(resp.name).toLocaleLowerCase().match(this.name6.toLocaleLowerCase()));
		}
	}
  ...
  export2() {
    ...
    if (this.plt.is('capacitor') || this.plt.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'super_fans.csv', csv, { replace: true })
        .then(res => {
  ...
  export() {
    ...
    // console.log(csv)
    if (this.plt.is('capacitor') || this.plt.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'top_listeners.csv', csv, { replace: true })
        .then(res => {
          this.socialSharing.share(null, null, res.nativeURL, null);
        }).catch();
    } else {
  ...
  export3() {
    ...
    // console.log(csv)
    if (this.plt.is('capacitor') || this.plt.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'new_fans.csv', csv, { replace: true })
        .then(res => {
          this.socialSharing.share(null, null, res.nativeURL, null);
        }).catch();
    } else {
  ...
  export4() {
    ...
    // console.log(csv)
    if (this.plt.is('capacitor') || this.plt.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'follwers_gained.csv', csv, { replace: true })
        .then(res => {
          this.socialSharing.share(null, null, res.nativeURL, null);
        }).catch();
    } else {
  ...
  export5() {
    ...
    // console.log(csv)
    if (this.plt.is('capacitor') || this.plt.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'moderators.csv', csv, { replace: true })
        .then(res => {
          this.socialSharing.share(null, null, res.nativeURL, null);
        }).catch();
    } else {
  ...
  export6() {
    ...
    // console.log(csv)
    if (this.plt.is('capacitor') || this.plt.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'speakers.csv', csv, { replace: true })
        .then(res => {
          this.socialSharing.share(null, null, res.nativeURL, null);
        }).catch();
    } else {
  ...
  export7() {
    ...
    // console.log(csv)
    if (this.plt.is('capacitor') || this.plt.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'Speakers_data.csv', csv, { replace: true })
        .then(res => {
          this.socialSharing.share(null, null, res.nativeURL, null);
        }).catch();
    } else {
  ...
  async getAppData(data, api_token) {
    this.superFans = [];
    const fields: string[] = [];
    this.app.getRoomDetails(data, api_token).subscribe((res: any) => {
      if (res.status === 'success') {
        ...
        // outputs: "48:39:30"
			...
			this.avgListend = `${minutes}M, ${rseconds}S`;
  ...
  async exportToHS() {
    ...
    await loading.present();
		...
		this.app.exporttohubsport(this.api_token, channel).subscribe(async (res: any) => {
